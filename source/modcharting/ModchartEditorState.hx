...
import funkin.play.notes.CastNote;
...
class ModchartEditorEvent extends FlxSprite implements IFlxDestroyable
{
	#if (PSYCH || LEATHER)
...
#if (PSYCH || LEATHER)
class ModchartEditorState extends #if (PSYCH && PSYCHVERSION >= "0.7") funkin.ui.MusicBeatState #else MusicBeatState #end
{
	...
	{
		#if PSYCH
		funkin.backend.CacheSystem.clearStoredMemory();
		funkin.backend.CacheSystem.clearUnusedMemory();
		#end
		#if (PSYCH && PSYCHVERSION >= "0.7.3")
		...
		strumLine = new FlxSprite(#if !(PSYCHVERSION >= "0.7") ClientPrefs.middleScroll #else ClientPrefs.data.middleScroll #end
			?funkin.Constants.STRUM_X_MIDDLESCROLL:funkin.Constants.STRUM_X, 50).makeGraphic(FlxG.width, 10);
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ESCAPE)
		{
			var exitFunc = function()
			{
				...
				#end
				funkin.ui.transition.LoadingState.loadAndSwitchState(new PlayState(null));
			};
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		var strumLineX:Float = #if (PSYCH && PSYCHVERSION >= "0.7") ClientPrefs.data.middleScroll ? funkin.Constants.STRUM_X_MIDDLESCROLL : funkin.Constants.STRUM_X #elseif (PSYCH
			&& PSYCHVERSION < "0.7") ClientPrefs.middleScroll ? funkin.Constants.STRUM_X_MIDDLESCROLL : funkin.Constants.STRUM_X #elseif LEATHER utilities.Options.getData("middlescroll") #end;
	...
	function getVocalFromCharacter(char:String)
	{
		try
		{
			var path:String = Paths.getPath(funkin.Constants.DEFAULT_CHARACTER_PATH + '$char.json', TEXT, null, true);
			if (!FileSystem.exists(path))
				path = Paths.getPath(funkin.Constants.PSYCH_CHARACTER_PATH + '$char.json', TEXT, null, true);
			#if MODS_ALLOWED
